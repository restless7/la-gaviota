...
export default function AdminDashboardClient({ 
  ...
  const getTierBadge = (clerk_user_id: string | null) => {
    if (!clerk_user_id) return <span className="bg-gray-100 text-gray-800 px-2 py-1 rounded-md text-[10px] font-bold uppercase">Retail</span>;
    return <span className="bg-blue-100 text-blue-800 px-2 py-1 rounded-md text-[10px] font-bold uppercase">Wholesale</span>;
  };
    ...
    <div className="space-y-8 animate-fade-in max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <StatCard
          ...
          value={`${initialLowStockItems.length} items`}
          icon={AlertTriangle}
          color="text-[#FFCC00]"
          bgColor="bg-[#FFCC00]"
        />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
         ...
         <div className="lg:col-span-2 space-y-6">
            ...
            {/* Recent B2B Orders Overview — from real database */}
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
               <div className="flex justify-between items-center mb-6">
                  <h3 className="font-bold text-lg text-slate-800">Últimos Pedidos Recibidos</h3>
                     ...
                     <tbody className="divide-y divide-gray-50">
                        {initialRecentOrders.map(order => (
                           <tr key={order.id} className="hover:bg-slate-50">
                              ...
                              <td className="px-4 py-3 font-bold text-slate-800">{order.customer_name}</td>
                              <td className="px-4 py-3">{getTierBadge(order.clerk_user_id)}</td>
