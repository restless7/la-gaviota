...
import { Search, Plus, MapPin, Phone, Truck, X, Loader2, Save } from 'lucide-react';
import { Supplier, createSupplier, updateSupplier } from '@/src/actions/suppliers';
    ...
    try {
      if (editingSupplier) {
        ...
        if (result.success && result.data) {
          setSuppliers(prev => prev.map(s => s.id === editingSupplier.id ? result.data as Supplier : s));
          setShowModal(false);
        }
      } else {
              ...
              {filtered.map(supplier => (
                <tr key={supplier.id} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-right">
                     <button onClick={() => openEditModal(supplier)} className="text-[#E30613] font-bold hover:underline text-xs">Editar</button>
                  </td>
                </tr>
              ))}
              {filtered.length === 0 && (
                <tr>
                  <td colSpan={5} className="p-12 text-center text-gray-400 font-medium italic">
                    No hay proveedores registrados en esta vista.
                  </td>
                </tr>
              )}
            </tbody>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setShowModal(false)}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg p-8" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-2xl font-black text-slate-800">{editingSupplier ? 'Editar Granja' : 'Registrar Nueva Granja'}</h2>
            ...
            <form onSubmit={handleSaveSupplier} className="space-y-4">
              <div>
                ...
                <input name="name" defaultValue={editingSupplier?.name || ''} required className="w-full border border-gray-200 rounded-lg px-4 py-2.5 outline-none focus:border-[#4CAF50]" placeholder="Finca El Ejemplo" />
              </div>
              <div>
                <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Ubicación (Vereda, Municipio) *</label>
                <input name="location" defaultValue={editingSupplier?.farm_location || ''} required className="w-full border border-gray-200 rounded-lg px-4 py-2.5 outline-none focus:border-[#4CAF50]" placeholder="Lebrija, Santander" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Persona de Contacto *</label>
                  <input name="contact" defaultValue={editingSupplier?.contact_name || ''} required className="w-full border border-gray-200 rounded-lg px-4 py-2.5 outline-none focus:border-[#4CAF50]" placeholder="Don Pedro" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Teléfono *</label>
                  <input name="phone" defaultValue={editingSupplier?.phone || ''} required className="w-full border border-gray-200 rounded-lg px-4 py-2.5 outline-none focus:border-[#4CAF50]" placeholder="315-123-4567" />
                </div>
              </div>
              <div>
                <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Email (opcional)</label>
                <input name="email" defaultValue={editingSupplier?.email || ''} type="email" className="w-full border border-gray-200 rounded-lg px-4 py-2.5 outline-none focus:border-[#4CAF50]" placeholder="proveedor@email.com" />
              </div>
              <div>
                <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Categorías Suministradas (separadas por coma) *</label>
                <input name="categories" defaultValue={editingSupplier?.supplied_categories?.join(', ') || ''} required className="w-full border border-gray-200 rounded-lg px-4 py-2.5 outline-none focus:border-[#4CAF50]" placeholder="Frutas, Verduras" />
              </div>
              {editingSupplier && (
                <div>
                  <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Estado de Operación *</label>
                  <select name="status" defaultValue={editingSupplier.status} required className="w-full border border-gray-200 rounded-lg px-4 py-2.5 outline-none focus:border-[#4CAF50] bg-white">
                    <option value="Activo">Activo (Despachando)</option>
                    <option value="Inactivo">Inactivo (Suspendido)</option>
                  </select>
                </div>
              )}
              {!editingSupplier && <input type="hidden" name="status" value="Activo" />}
              <div className="flex gap-3 pt-4">
                <button type="button" onClick={() => setShowModal(false)} className="flex-1 border border-gray-200 text-gray-600 py-2.5 rounded-lg font-bold hover:bg-slate-50 transition-colors">
                   Cancelar
                </button>
                <button type="submit" disabled={isSubmitting} className="flex-1 bg-[#4CAF50] hover:bg-green-700 text-white py-2.5 rounded-lg font-bold shadow-md flex items-center justify-center gap-2 transition-colors">
                   {isSubmitting ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />}
                   {isSubmitting ? 'Guardando...' : (editingSupplier ? 'Guardar Cambios' : 'Registrar Granja')}
                </button>
              </div>
            </form>
          </div>
